...
if history == nil then
	if history_path then
		mp.msg.warn(
			"cannot write to history file " .. options.history_path .. ", new entries will not be saved to disk"
		)
...
function unaccent(str)
	...
	local charmap = "AÀÁÂÃÄÅĀĂĄǍǞǠǺȀȂȦȺAEÆǢǼ"
		.. "BßƁƂƄɃ"
		.. "CÇĆĈĊČƆƇȻ"
		.. "DÐĎĐƉƊDZƻǄǱDzǅǲ"
		.. "EÈÉÊËĒĔĖĘĚƎƏƐȄȆȨɆ"
	...
	local function select_item(append)
		...
		if append and item.value[1] == "loadfile" then
			...
			for i = 1, #playlist do
				...
				local display_path, save_path, effective_path, effective_protocol, is_remote, file_options =
					path_info(playlist_file)
...
function draw_menu()
	...
	if #menu_data.items > 0 then
		...
		for i = 1, #menu_data.items do
			...
			if item.title then
				local icon
				local separator = last_state.selected_index == i and "{\\alpha&HFF&}●{\\alpha&H00&}  - "
					or "{\\alpha&HFF&}●{\\alpha&H00&} - "
...
function get_full_path()
	...
	if path == nil or path == "-" or path == "/dev/stdin" then
		return
	end
...
function path_info(full_path)
	local function resolve(effective_path, save_path, display_path, last_protocol, is_remote)
		...
		if protocol == "archive" then
			...
			if not main_path then
				...
				_, save_path, effective_path, protocol, is_remote, file_options =
					resolve(effective_path, save_path, display_path, protocol, is_remote)
				...
				display_path, save_path, _, protocol, is_remote, file_options =
					resolve(main_path, save_path, main_path, protocol, is_remote)
				effective_path = normalize(display_path)
				save_path = save_path or effective_path
				save_path = "archive://"
					.. save_path
					.. (save_path:find("archive://") and archive_path:gsub("|", "%%7C") or archive_path)
					.. filename
	...
	local display_path, save_path, effective_path, effective_protocol, is_remote, file_options =
		resolve(nil, nil, full_path, nil, false)
...
function write_history(display)
	...
	elseif effective_protocol == "dvd" or effective_protocol == "dvdnav" then
		full_path = full_path
			.. " --opt=dvd-angle="
			.. mp.get_property("dvd-angle", "1")
			.. ",dvd-device="
			.. mp.get_property("dvd-device", "")
	end
...
function show_history(entries, next_page, prev_page, update, return_items)
	...
	local menu_items = {}
	local state = (prev_page or next_page) and last_state
		or {
			known_dirs = {},
			known_files = {},
			existing_files = {},
	...
	while #menu_items < entries do
		...
		if
			not return_items
			and (attempts > 0 or not (prev_page or next_page))
			and attempts % options.entries == 0
			and #menu_items ~= item_count
		then
...
function dyn_menu_update()
	...
	if not options.enabled then
		menu.submenu =
			{ { title = "Add current file to memo", cmd = "script-binding memo-log" }, { type = "separator" } }
	end
...
mp.add_key_binding(nil, "memo-last", function()
	...
	if
		last_state
		and last_state.current_page == 1
		and options.hide_duplicates
		and options.hide_deleted
		and options.entries >= 2
		and not search_words
		and not dir_menu
	then
