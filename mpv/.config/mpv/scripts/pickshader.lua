...
--
-- Shift+ENTER will REMOVE the selected shader from --glsl-shaders
--
...
local o = {
	shader_dir = "~~/shaders",
	-- font size (ASS points). Can be overridden in ~~/script-opts/pickshader.conf
	font_size = 26,
	-- color for shaders already applied (ASS BGR hex, no &H or &). Example: "00FF00" is green
	applied_color = "00FF00",
	-- default color for non-applied lines (ASS BGR hex)
...
local function draw()
	...
	local results_str = ""
	-- build map of applied shaders from mpv's glsl-shaders list
	local applied_map = {}
	local applied = mp.get_property_native("glsl-shaders") or {}
	local shader_dir = mp.command_native({ "expand-path", o.shader_dir })
	for _, entry in pairs(applied) do
		local basename = entry
		if shader_dir and entry:sub(1, shader_dir:len() + 1) == shader_dir .. "/" then
			basename = entry:sub(shader_dir:len() + 2)
		else
			basename = entry:match("[^/\\]+$") or entry
		end
		applied_map[basename] = true
	end

	-- reorder grepped_arr so applied shaders appear first (alphabetical within each group)
	table.sort(grepped_arr, function(a, b)
		local aa = applied_map[a] and 1 or 0
		local bb = applied_map[b] and 1 or 0
		if aa ~= bb then
			return aa > bb
		end
		return a < b
	...
	local fs_tag = "{\\fs" .. o.font_size .. "}"
	local color_tag = "{\\c&H" .. o.applied_color .. "&}"
	local default_color_tag = "{\\c&H" .. o.default_color .. "&}"

	for i, shader in pairs(grepped_arr) do
		if math.abs(i - cursor) < 4 then
			local display = (i == cursor) and "> " .. shader or shader
			-- lazy underlining, apply to display only so color tags don't get matched
			local display_for_add = display
			if current_pattern ~= "" then
				local e, underlined = pcall(string.gsub, display_for_add, current_pattern, "{\\u1}%0{\\u0}")
				if e then
					display_for_add = underlined
				end
				local e2, underlined2 = pcall(string.gsub, display_for_add, current_pattern:upper(), "{\\u1}%0{\\u0}")
				if e2 then
					display_for_add = underlined2
				end
			end
			local tag = fs_tag .. (applied_map[shader] and color_tag or default_color_tag)
	...
	if results_str == "" then
		results_str = fs_tag .. default_color_tag .. "> " .. current_pattern
	end

	-- convert literal newlines to ASS line breaks (lines already contain tags)
	local ass_results = results_str:gsub("\n", "\\N")
	overlay.data = ass_results
	overlay:update()
end
...
local function pickshader()
	if not mode_on then
		...
		mp.add_forced_key_binding("Ctrl+n", "incr1", incr_cursor, { repeatable = true })
		mp.add_forced_key_binding("Ctrl+p", "decr1", decr_cursor, { repeatable = true })
		mp.add_forced_key_binding("DOWN", "incr2", incr_cursor, { repeatable = true })
		mp.add_forced_key_binding("UP", "decr2", decr_cursor, { repeatable = true })
		mp.add_forced_key_binding("BS", "backspace", backspace, { repeatable = true })
		mp.add_forced_key_binding("Ctrl+u", "clear", clear)
		mp.add_forced_key_binding("ENTER", "pick_add", pick)
		mp.add_forced_key_binding("Shift+ENTER", "pick_remove", function()
			pick("remove")
		end)
		mp.add_forced_key_binding("Ctrl+ENTER", "pick_set", function()
			pick("set")
		end)
		mp.add_forced_key_binding("any_unicode", "handle_input", handle_input, { repeatable = true, complex = true })
		draw()
	else
		incr_cursor()
	end
...
mp.add_key_binding("Ctrl+r", "pickshader", pickshader, { repeatable = true })
